...
<%
	...
	// 디렉토리가 없을 경우 생성하기
	if(!targetDir.exists()) {//디렉토리가 없니?
	 	targetDir.mkdirs(); //디렉토리 만들어라.
	}
	...
	//끝에 new DefaultFileRenamePolicy() :중복된 파일이 있을 경우 1,2,3,,, 이런 식으로 붙여준다.
	//파일 말고 나머지만 가져온다.
	Enumeration params = multi.getParameterNames(); // 키만 떼옴(네임속성 이름)
	while (params.hasMoreElements()) {
		String name = (String) params.nextElement();
		String value = multi.getParameter(name); // 값을 떼온다.
		out.println(name + " = " + value + "<br>");
	}
	out.println("-----------------------------------<br>");

	Enumeration files = multi.getFileNames(); // 파일명 떼오기

	 while (files.hasMoreElements()) {
		String name = (String) files.nextElement(); // 파라미터 이름 filename
		String original = multi.getOriginalFileName(name); // 실제 원본 파일 이름
		String filename = multi.getFilesystemName(name); // 바뀐 파일명
		String type = multi.getContentType(name); // 데이터의 파일확장명
		File file = multi.getFile(name); //파일 객체 가져옴(정보 가져옴). 가져올 때 getFile을 이용해서 각각의 그 파일이름으로 가져올 수 있다.
