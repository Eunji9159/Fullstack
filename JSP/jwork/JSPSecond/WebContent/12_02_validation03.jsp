...
<script>
	function checkMember() {
		...
		var regExpId = /^[0-9a-zA-Z]{4,8}/; //숫자,영문자 4자~8자 이내 가능   '^' : 처음시작하는 단어가~~로 시작되어야 한다.
		var regExpName = /^[가-힣]*$/; //$ : 한글 문자만 가능. ㄱ~ㅎ, ㅏ~ㅣ도 안됨(자음이나 모음만 오는 것도 안됨. 한 단어로 와야함!).
		var regExpPasswd = /^[0-9a-zA-Z]{4,12}/; //숫자,영문자 4자~12자 이내 가능
		var regExpPhone = /^\d{3}-\d{4}-\d{4}$/; //숫자3-숫자4-숫자4개 형식으로만 가능 '\d' : 숫자 표현.  '-' : 대시(-) 형식 넣어야 한다.
		...
		if (!regExpId.test(uid)) {
			...
			fm.uid.select(); // 포커스 기능 : 커서 기능(깜박깜박)
			return;
		}
